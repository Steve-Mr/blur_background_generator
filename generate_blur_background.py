...
def process_image(img_path, width, height, radius):
...
def process_and_overlay_image(img_path, width, height, radius, zoom, contrast):
    ...
    background = process_image(img_path, width, height, radius)
    ...
    if img_ratio >= target_ratio:
        ...
        enhanced_background.paste(resized_overlay, (x_offset, y_offset))
    else:
        # Height is covering the target area, calculate the width
